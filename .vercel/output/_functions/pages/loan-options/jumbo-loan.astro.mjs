...
var _a$1;
const $$FAQSectionJumbo = createComponent(($$result, $$props, $$slots) => {
  const faqItems = [
    { question: "What is a jumbo loan?", answer: "A jumbo loan exceeds conforming loan limits set by FHFA. It helps finance high-value homes that require larger loan amounts than conventional limits." },
    { question: "How much down payment is required?", answer: "Down payments typically range from 10% to 20%+ depending on borrower profile, property type, and loan amount." },
    { question: "What credit score do I need?", answer: "Jumbo loans often require strong credit, typically 700+, though some programs allow lower with compensating factors." },
    { question: "Are reserve funds required?", answer: "Yes, jumbo programs commonly require several months of reserves (post-closing liquidity) to strengthen the file." },
    { question: "Do jumbo loans allow second homes or investment properties?", answer: "Many jumbo programs allow primary, second home, and investment purchases, with stricter requirements for non-primary occupancy." },
    { question: "Is an appraisal different for jumbo?", answer: "Expect a high-quality appraisal; some lenders require two appraisals for certain properties or price points." },
    { question: "Fixed vs ARM for jumbo?", answer: "Both are available. ARMs can offer lower initial rates. The right choice depends on timelines and risk preferences." }
  ];
  return renderTemplate(_a$1 || (_a$1 = __template$1(["", `<section class="py-16 bg-white"> <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8"> <div class="grid grid-cols-1 lg:grid-cols-12 gap-12"> <div class="lg:col-span-4"> <h2 class="text-3xl font-bold text-gray-900 mb-4">FAQ's about Jumbo loans</h2> <p class="text-gray-600">Guidance for financing high-value properties.</p> </div> <div class="lg:col-span-8" id="faq-accordion"> <div class="space-y-4"> `, " </div> </div> </div> </div> <script>\n    document.addEventListener('DOMContentLoaded', function() {\n      const accordion = document.getElementById('faq-accordion');\n      if (!accordion) return;\n      accordion.addEventListener('click', function(e) {\n        const question = e.target.closest('.faq-question');\n        if (!question) return;\n        e.preventDefault();\n        const index = question.getAttribute('data-index');\n        const content = document.getElementById(`faq-answer-${index}`);\n        const icon = question.querySelector('.faq-icon');\n        if (!content || !icon) return;\n        const isExpanded = question.getAttribute('aria-expanded') === 'true';\n        const allContents = accordion.querySelectorAll('.faq-content');\n        const allIcons = accordion.querySelectorAll('.faq-icon');\n        const allQuestions = accordion.querySelectorAll('.faq-question');\n        allContents.forEach((item, i) => {\n          if (item.id !== `faq-answer-${index}`) {\n            item.style.display = 'none';\n            allIcons[i].classList.remove('rotate-180');\n            allQuestions[i].setAttribute('aria-expanded', 'false');\n          }\n        });\n        if (isExpanded) {\n          content.style.display = 'none';\n          icon.classList.remove('rotate-180');\n          question.setAttribute('aria-expanded', 'false');\n        } else {\n          content.style.display = 'block';\n          icon.classList.add('rotate-180');\n          question.setAttribute('aria-expanded', 'true');\n        }\n      });\n    });\n  <\/script> </section>"], ["", `<section class="py-16 bg-white"> <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8"> <div class="grid grid-cols-1 lg:grid-cols-12 gap-12"> <div class="lg:col-span-4"> <h2 class="text-3xl font-bold text-gray-900 mb-4">FAQ's about Jumbo loans</h2> <p class="text-gray-600">Guidance for financing high-value properties.</p> </div> <div class="lg:col-span-8" id="faq-accordion"> <div class="space-y-4"> `, " </div> </div> </div> </div> <script>\n    document.addEventListener('DOMContentLoaded', function() {\n      const accordion = document.getElementById('faq-accordion');\n      if (!accordion) return;\n      accordion.addEventListener('click', function(e) {\n        const question = e.target.closest('.faq-question');\n        if (!question) return;\n        e.preventDefault();\n        const index = question.getAttribute('data-index');\n        const content = document.getElementById(\\`faq-answer-\\${index}\\`);\n        const icon = question.querySelector('.faq-icon');\n        if (!content || !icon) return;\n        const isExpanded = question.getAttribute('aria-expanded') === 'true';\n        const allContents = accordion.querySelectorAll('.faq-content');\n        const allIcons = accordion.querySelectorAll('.faq-icon');\n        const allQuestions = accordion.querySelectorAll('.faq-question');\n        allContents.forEach((item, i) => {\n          if (item.id !== \\`faq-answer-\\${index}\\`) {\n            item.style.display = 'none';\n            allIcons[i].classList.remove('rotate-180');\n            allQuestions[i].setAttribute('aria-expanded', 'false');\n          }\n        });\n        if (isExpanded) {\n          content.style.display = 'none';\n          icon.classList.remove('rotate-180');\n          question.setAttribute('aria-expanded', 'false');\n        } else {\n          content.style.display = 'block';\n          icon.classList.add('rotate-180');\n          question.setAttribute('aria-expanded', 'true');\n        }\n      });\n    });\n  <\/script> </section>"])), maybeRenderHead(), faqItems.map((item, index) => renderTemplate`<div class="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden"> <button type="button" class="faq-question w-full px-6 py-4 text-left flex items-center justify-between hover:bg-gray-50 transition-colors"${addAttribute(index, "data-index")} aria-expanded="false"${addAttribute(`faq-answer-${index}`, "aria-controls")}> <span class="font-semibold text-gray-900">${item.question}</span> <svg class="faq-icon w-5 h-5 text-gray-400 transform transition-transform duration-200" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg> </button> <div${addAttribute(`faq-answer-${index}`, "id")} class="faq-content px-6 py-0 border-t border-gray-100" style="display: none;"> <div class="py-4 text-gray-600">${item.answer}</div> </div> </div>`));
}, "/Users/pamplona/Documents/trae_projects/broker-lenders/src/components/FAQSectionJumbo.astro", void 0);

const title$1 = "Jumbo Loan - Broker Lenders";
const breadcrumb$1 = {"home":"Home","loanOptions":"Loan options","jumboLoan":"Jumbo loan"};
const hero$1 = {"title":"Jumbo loan","subtitle":"Finance high-value properties with competitive rates and flexible terms.","getStarted":"Get started"};
const whyChoose$1 = {"title":"Why choose Broker Lenders for jumbo loans?","highLoanAmounts":{"title":"High loan amounts","description":"Finance properties above conforming loan limits with confidence."},"flexibleTerms":{"title":"Flexible terms and options","description":"Choose from fixed or adjustable rates to fit your financial plans."},"personalizedSupport":{"title":"Personalized support","description":"Work with specialists experienced in high-value transactions."},"getStarted":"Get started"};
const answersSection$1 = {"title":"Answers when you need them.","subtitle":"Real people when it matters.","description":"Get assistance with complex eligibility, jumbo underwriting, and rate strategies.","getStarted":"Get started"};
const finalCTA$1 = {"title":"Move forward with a Jumbo loan","getStarted":"Get started","callNow":"Call (801) 307-0273"};
const enDict = {
  title: title$1,
...
const title = "Préstamo Jumbo - Broker Lenders";
const breadcrumb = {"home":"Inicio","loanOptions":"Opciones de préstamos","jumboLoan":"Préstamo jumbo"};
const hero = {"title":"Préstamo jumbo","subtitle":"Financia propiedades de alto valor con tarifas competitivas y términos flexibles.","getStarted":"Comenzar"};
const whyChoose = {"title":"¿Por qué elegir Broker Lenders para préstamos jumbo?","highLoanAmounts":{"title":"Montos de préstamo altos","description":"Financia propiedades por encima de los límites de préstamos conformes con confianza."},"flexibleTerms":{"title":"Términos y opciones flexibles","description":"Elige entre tasas fijas o ajustables para adaptarse a tus planes financieros."},"personalizedSupport":{"title":"Apoyo personalizado","description":"Trabaja con especialistas con experiencia en transacciones de alto valor."},"getStarted":"Comenzar"};
const answersSection = {"title":"Respuestas cuando las necesitas.","subtitle":"Personas reales cuando importa.","description":"Obtén asistencia con elegibilidad compleja, suscripción jumbo y estrategias de tasas.","getStarted":"Comenzar"};
const finalCTA = {"title":"Avanza con un préstamo Jumbo","getStarted":"Comenzar","callNow":"Llamar (801) 307-0273"};
const esDict = {
  title,
  breadcrumb,
  hero,
...
const $$JumboLoan = createComponent(($$result, $$props, $$slots) => {
  const Astro2 = $$result.createAstro($$Astro, $$props, $$slots);
  Astro2.self = $$JumboLoan;
  ...
  return renderTemplate(_a || (_a = __template(["<!-- i18n dynamic translations for Jumbo Loan page -->", '<div id="i18n-jumbo"', "", ` style="display:none"></div> <script>
  (() => {
    const root = document.getElementById('i18n-jumbo');
    const decode = (val) => {
      try {
        const b64 = val || '';
        const bin = atob(b64);
